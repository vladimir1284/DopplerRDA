  ...
  if Celdas1.Checked
    then Result := IntToStr(Cell)
           ...
           then Result := Format('%.1f km', [Cell * Light_Speed/(2*Video.Frecuencia_AD)])
                  ...
                  then Result := Format('%.0f µs', [Cell * 1000000 / Video.Frecuencia_AD]);
end;
  ...
  if fAdjust
    then
      begin
        ...
        UpDown1.Max := pred(CellCount);
        UpDown2.Max := pred(CellCount);
...
begin
  if Acquiring_dBT_DBZ or Acquiring_dBZ_DBZ
    then
      begin
        GetMem(Buffer, SizeOf(single) * AcqCells);
        try
          FillChar(Buffer^, SizeOf(single) * AcqCells, 0);
      ...
      if(channel = 0)
         then Video1.SetData1(Data^)
         else Video1.SetData2(Data^);
end;
