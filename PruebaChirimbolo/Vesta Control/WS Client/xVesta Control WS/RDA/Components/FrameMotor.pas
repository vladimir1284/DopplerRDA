...
type
  ...
  private
    ...
    function GetMotorControl : IMotorWS;
    function GetObservations: IObservationsWS;
    function GetMotor: IMotorWS;
  public
    procedure Init(_Motor: IMotorWS; _Observations: IObservationsWS; AuthHeader : TAuthHeader);
    procedure UpdateView;

    property Updating : boolean read fUpdating;
  published
    property Motor        : IMotorWS read GetMotor;
    property Control      : IMotorWS read GetMotorControl;
...
begin
  ...
  if assigned(fMotor)
    then
      begin
        ...
        try
          try
            Bulb3.State := Motor.Lazo_Sector;
            Bulb1.State := CheckBox1.Checked and (Abs( Motor.Posicion - Motor.SP_Posicion ) <= ShellClient.Configuration.Configuration.Angle_Accuracy);
            Bulb2.State := (Bulb3.State and (Abs( Abs(Motor.Velocidad) - Abs(Motor.SP_Velocidad) ) <= ShellClient.Configuration.Configuration.Speed_Accuracy)) or
                           (CheckBox2.Checked and (Abs( Motor.Velocidad - Motor.SP_Velocidad ) <= ShellClient.Configuration.Configuration.Speed_Accuracy));
            Antenna.Command  := Motor.SP_Posicion;
  ...
  if assigned(fObservations)
    then SpeedButton1.Enabled := assigned(Observations.Buscar(fTemplateName));
end;
...
begin
  if assigned(Observations)
    then Observations.Ejecutar(fTemplateName);
end;
    ...
    then
      with TMotorCalib_Dialog.Create(Self) do
        try
          ...
          Edit1.Text := Format('%.1f', [Motor.Lazo_Posicion_K]);
          Edit2.Text := Format('%.1f', [Motor.Lazo_Velocidad_K]);
          if ShowModal = ID_OK
            then
              begin
                try
                  Control.Lazo_Posicion_K := StrToFloat(Edit1.Text);
                except on EConvertError do
                end;
                try
                  Control.Lazo_Velocidad_K := StrToFloat(Edit2.Text);
                except on EConvertError do
                end;
              end;
        finally
...
procedure TFrame_Motor.Init(_Motor: IMotorWS; _Observations: IObservationsWS; AuthHeader : TAuthHeader);
begin
  fUpdating := false;
  fAuthHeader := AuthHeader;
  fMotor := _Motor;
  fObservations := _Observations;
...
function TFrame_Motor.GetMotorControl: IMotorWS;
var
  Headers : ISOAPHeaders;
begin
  Headers := fMotor as ISOAPHeaders;
  Headers.Send(fAuthHeader);
  result := fMotor;
end;

function TFrame_Motor.GetObservations: IObservationsWS;
var
  Headers : ISOAPHeaders;
begin
...
function TFrame_Motor.GetMotor: IMotorWS;
begin
  result := fMotor;
end;
