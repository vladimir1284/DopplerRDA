  ...
  TLog =
    class
      ...
      public
        ...
        procedure   SaveLogs;
        procedure   AddLogMessage( const ADateTime : TDateTime; const ACategory : TLogCategory; const AnUser, AZone, ATitle, AMessage : string );
...
uses
  SysUtils, DataUtils, IdSMTP,
  Radars, Description, Config, Elbrus, IdMessage;

{ TLog }

procedure TLog.ConfigureServer;
begin
  with Utils.SMTPServer, theConfiguration do
  begin
...
procedure TLog.AddLogMessage( const ADateTime : TDateTime; const ACategory : TLogCategory; const AnUser, AZone, ATitle, AMessage : string );
begin
  AddLogRawMessage( ADateTime, ACategory, AnUser, AZone, ATitle, AMessage );
  if (ACategory = lcError) and (theConfiguration.SendMsgOnError)
    then SendErrorEmail( ATitle, AMessage );
{
  if (LogCategory = lcWarning) and (theConfiguration.SendMsgOnError)
    then SendWarningEmail( LogMsgSection, LogMsg );
}    
end;
...
var
  ...
  i      : integer;
begin
  with Utils.SMTPServer, Utils.EMailMessage do
  try
    ConfigureServer;
