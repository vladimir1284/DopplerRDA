...
uses
  ...
  Video, VersionInfo, DataUtils, Constants, StatisticsManager;
    ...
    then
      if Users.DeleteUser(Name)
        then
          begin
            Result := S_OK;
            LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'El usuario ' + fName + ' borro al usuario ' + Name, 'El usuario ' + fName + ' borro al usuario ' + Name  );
          end
        else Result := E_FAIL
    ...
    if fLevel > ul_NotLogged
      then
        begin
          ...
          fPwd  := Password;
          Result := S_OK;
          LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'Se conecto al sistema el usuario ' + fName, 'Se conecto al sistema el usuario ' + fName );
        end
      else Result := E_ACCESSDENIED;
  finally
    fReady.Release;
...
begin
  fReady.Acquire;
  try
    LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'Se desconecto del sistema el usuario '+fName, 'Se desconecto del sistema el usuario '+fName );
    ReleaseControl;
    if fLevel > ul_NotLogged
      then Result := S_OK
      else Result := E_FAIL;
  ...
  if Get_InControl
    then
      begin
        ...
        if Result = S_OK
          then LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'El usuario '+fName+' modifico al usuario ' + Name, 'El usuario '+fName+' modifico al usuario ' + Name  );
      end
    else Result := E_ACCESSDENIED;
end;
    ...
    then
      if Users.CreateUser(Name, Password, Level)
        then
          begin
            Result := S_OK;
            LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'El usuario '+fName+' creo al usuario '+Name, 'El usuario '+fName+' creo al usuario '+Name );
          end
        else Result := E_FAIL
    else Result := E_ACCESSDENIED;
end;
      ...
      then
        if Users.ResetController(fName, fPwd)
          then
            begin
              Result := S_OK;
              LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'El usuario '+fName+' libero el control', 'El usuario '+fName+' libero el control' );
            end
          else Result := E_FAIL
      else Result := E_ACCESSDENIED;
  finally
  ...
  try
    if Users.SetController(fName, fPwd)
      then
        begin
          Result := S_OK;
          LogMessages.AddLogMessage( Now, lcInformation, fName, LoggingZone, 'El usuario '+fName+' tomo el control', 'El usuario '+fName+' tomo el control' );
        end
      else Result := E_FAIL;
  finally
    fReady.Release;
