    ...
    then
      if Users.DeleteUser(Name)
        then
          begin
            Result := true;
            LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'El usuario ' + UserName + ' borro al usuario ' + Name, 'El usuario ' + UserName + ' borro al usuario ' + Name  );
          end;
end;
    ...
    if UserLevel > ul_NotLogged
      then
        begin
          ...
          LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'Se conecto al sistema el usuario ' + UserName, 'Se conecto al sistema el usuario ' + UserName );
        end;
  finally
    fReady.Release;
  end;
...
begin
  ...
  try
    ...
    LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'Se desconecto del sistema el usuario '+UserName, 'Se desconecto del sistema el usuario '+UserName );
    ReleaseControl;
  ...
  if InControl
    then
      begin
        ...
        if Result
          then LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'El usuario '+UserName+' modifico al usuario ' + Name, 'El usuario '+UserName+' modifico al usuario ' + Name  );
      end;
end;
    ...
    then
      if Users.CreateUser(Name, Password, Level)
        then
          begin
            Result := true;
            LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'El usuario '+UserName+' creo al usuario '+Name, 'El usuario '+UserName+' creo al usuario '+Name );
          end;
end;
      ...
      then
        if Users.ResetController(UserName, UserPassword)
          then
            begin
              Result := true;
              LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'El usuario '+UserName+' libero el control', 'El usuario '+UserName+' libero el control' );
            end;
  finally
    fReady.Release;
  end;
  ...
  try
    if Users.SetController(UserName, UserPassword)
      then
        begin
          Result := true;
          LogMessages.AddLogMessage( lcInformation, UserName, LoggingZone, 'El usuario '+UserName+' tomo el control', 'El usuario '+UserName+' tomo el control' );
        end
      else Result := false;
  finally
    fReady.Release;
