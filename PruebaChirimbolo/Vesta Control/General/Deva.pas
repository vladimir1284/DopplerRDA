...
uses
  Encoder, Parameters, Constants, Config;
...
begin
  ...
  if Comp2_X
    then Result := (not Result + 1) and $3FFF
    else Result := result and $3FFF;
end;
...
begin
  ...
  if Result >= AntennaZeroValue
    then dec(Result, AntennaMaxValue);
end;
...
var
  Code : longint;
begin
  X_Channel := theConfiguration.X_Channel;
  Y_Channel := theConfiguration.Y_Channel;
  Z_Channel := theConfiguration.Z_Channel;
