...
,	_sqrt2_2 (flt_t (sqrt (2) * 0.5))
{
	...
	if (_nbr_bits > 2)
	{
      ...
      DataIn= new flt_t [_length];
      FFTOut= new flt_t [_length];
      AbsOut= new flt_t [_length/2];
      FFTLock= new TCriticalSection;
      Done=false;
}
/*==========================================================================*/
/*      Name: Destructor                                                    */
...
unsigned int FFTReal::get_length()
{
   return _length;
}
...
void	FFTReal::do_fft ()
{
  ...
  do_fft (FFTOut, DataIn);

}

void FFTReal::CalcAbs()
{
	AbsOut[0] = 10.0*log10(FFTOut[0]);
	double img=0;

    FFTPeakVal=-9999999999999;
    FFTMinVal=1e100;
    for (int i = 1; i < _length/2; i ++)
    {
       double  img;
       const double real = FFTOut[i];
       img = FFTOut[i + _length/2];
	   AbsOut[i] = sqrt (real * real + img * img);
	   AbsOut[i]= 10.0*log10(AbsOut[i]);
	   if(AbsOut[i]>FFTPeakVal)
       {
          FFTPeakVal=AbsOut[i];
          FFTPeakBin=i;
       }
       if(AbsOut[i]<FFTMinVal)
       {
          FFTMinVal=AbsOut[i];
          FFTMinBin=i;
       }
