...
var
  ...
  Buffer: TBytes;
begin
  if ((Index = INDEX_ContentLength) or
      (Index = INDEX_Content)) then
  begin
    if FRequestInfo.CommandType = hcPOST then
    begin
      Encoding := nil;
      FRequestInfo.PostStream.Position := 0;
      Size := FRequestInfo.PostStream.Size - FRequestInfo.PostStream.Position;
      SetLength(Buffer, Size);
      FRequestInfo.PostStream.ReadBuffer(Pointer(Buffer)^, Size);
