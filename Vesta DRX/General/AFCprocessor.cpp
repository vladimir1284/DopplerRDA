			...
			if(fReadThread->BufferFull[CurrentBuffer]){
				  if(WaitForStalo || ThisSite->TheRadar->StaloChanged() || ThisSite->TheRadar->SyncChanged()){
					  ...
					  if(fDataValid){
						  ...
						  if ((FreqEst == 30142858) || (FreqEst == 29857142) || (FreqEst == 30285714) || (FreqEst == 29714286)) {
							 FreqEst = IF;
						  }
...
void __fastcall AFCProcessingThread::OutputValues(INT64 TxFreq){
	if(!ThisSite->TheRadar->AFCChangesInhibited){
	//	ThisSite->TheRadar->SetNCOFrequency(CurrentIFEstimate);       // esto estaba comentariado
		INT64 STALOTargetFreq = TxFreq - IF;
	   /*	INT64 STALOTargetFreqErr = TxFreq - IF;
		if (STALOTargetFreq > 0) {
		 STALOTargetFreq = ThisSite->TheRadar->GetSTALOFrequency() - STALOTargetFreq;
		}
		else{
		  if (STALOTargetFreq < 0) {
		  STALOTargetFreq = ( ThisSite->TheRadar->GetSTALOFrequency() + STALOTargetFreq);
		  //STALOTargetFreq = STALOTargetFreq * -1;
		  }

		}
		if((abs(TxFreq - IF) >= FrequencyError) && (abs(TxFreq - IF) < 1000000) ){
...
INT64 __fastcall AFCProcessingThread::UpdateFreqEst(INT64 IFEstimate, INT64 *TxEstimateArray, INT64 *IFEstimateArray, int NEstimates){
	...
	IFEstimateArray[NEstimates] = IFEstimate;
	if (IFEstimate > 0) {
	 TxEstimateArray[NEstimates] = IFEstimate + + ThisSite->TheRadar->GetSTALOFrequency();//ThisSite->Stalo_Frequency;
	}
	else{
