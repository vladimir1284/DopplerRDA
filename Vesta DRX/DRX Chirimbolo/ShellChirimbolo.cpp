...
void TForm1::SetSync()
{
	...
	if(!ThisSite->TheRadar->SyncChanging()){
		...
		//By Robert
		ThisSite->TheRadar->setRequestedFrecuency(StrToInt(FrecuencyHV ->Text));
		ThisSite->TheRadar->setRequestedDelayTxMA(StrToFloat(EdDelayTxMA ->Text));
		ThisSite->TheRadar->setRequestedWidthTxMA(StrToFloat(EdWidthTxMA ->Text));
		ThisSite->TheRadar->setRequestedDelayRFG(StrToFloat(EdDelayRFG->Text));
		ThisSite->TheRadar->setRequestedWidthRFG(StrToFloat(EdWidthRFG->Text));
		ThisSite->TheRadar->setRequestedDelaySW(StrToFloat(EdDelaySW->Text));
		ThisSite->TheRadar->setRequestedWidthSW(StrToFloat(EdWidthSW->Text));
	...
	if(!IQ_DisplayPause && (GetTickCount() - StartTime > 500)){
		...
		switch (IQ_Charts->ActivePageIndex) {
			...
			case 5:
					...
					for(int i=0; i<ndata; i++){
						if(SQI[i] != Sample_Excluded)
							chartSQI->Series[0]->AddY(SQI[i], "", clRed);
						else
							chartSQI->Series[0]->AddY(0, "", clRed);
					}
					break;
			case 6:
					chartLOG->Series[0]->Clear();
					for(int i=0; i<ndata; i++){
						if(LOG[i] != Sample_Excluded)
							chartLOG->Series[0]->AddY(LOG[i], "", clRed);
						else
							chartLOG->Series[0]->AddY(0, "", clRed);
					}
					break;
			case 7:
					chartSIG->Series[0]->Clear();
					for(int i=0; i<ndata; i++){
						if(SIG[i] != Sample_Excluded)
							chartSIG->Series[0]->AddY(SIG[i], "", clRed);
						else
							chartSIG->Series[0]->AddY(0, "", clRed);
					}
					break;
			case 8:
					chartCCOR->Series[0]->Clear();
					for(int i=0; i<ndata; i++){
						if(CCOR[i] != Sample_Excluded)
							chartCCOR->Series[0]->AddY(CCOR[i], "", clRed);
						else
							chartCCOR->Series[0]->AddY(0, "", clRed);
					}
					break;
		}
		StartTime = GetTickCount();
...
void __fastcall TForm1::ScrDelayTxMAChange(TObject *Sender)   //By Robert
{
   float x;
   float y;
	x= float(ScrDelayTxMA->Position);
	y = x/10;
	EdDelayTxMA->Text = FormatFloat("0.00", y);

 // EdDelayTxMA->Text = FlotToStr(ScrDelayTxMA->Position);       //By Robert

}
//---------------------------------------------------------------------------


void __fastcall TForm1::ScrWidthTxMAChange(TObject *Sender)     //By Robert
{
  float x;
   float y;
   x= float(ScrWidthTxMA->Position);
   y = x/10;
   EdWidthTxMA ->Text  = FormatFloat("0.00", y);
  // EdWidthTxMA ->Text = IntToStr(ScrWidthTxMA->Position);        // By Robert
}
//---------------------------------------------------------------------------

void __fastcall TForm1::ScrDelayRFGChange(TObject *Sender)        //By Robert
{
	float x;
	float y;
	x= float(ScrDelayRFG->Position);
	y= x/10;
	EdDelayRFG->Text = FormatFloat("0.00", y);       //By Robert
}
//---------------------------------------------------------------------------

void __fastcall TForm1::ScrDelaySWChange(TObject *Sender)     //By Robert
{
   float x;
   float y;
   x= float(ScrDelaySW->Position);
   y = x/10;
 //  EdDelaySW ->Text  = FloatToStr(y);
	EdDelaySW ->Text = FormatFloat("0.00", y);

}
//---------------------------------------------------------------------------

void __fastcall TForm1::ScrWidthRFGChange(TObject *Sender)
{
   float x;
   float y;
   x = float(ScrWidthRFG->Position);
   y = x/10;
   EdWidthRFG ->Text =FormatFloat("0.00", y);
}
//---------------------------------------------------------------------------

void __fastcall TForm1::ScrWidthSWChange(TObject *Sender)
{
   float x;
   float y;
   x = float(ScrWidthSW->Position);
   y = x/10;
   EdWidthSW ->Text = FormatFloat("0.00", y);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::rgPLPCClick(TObject *Sender)
{
  if (rgPLPC->ItemIndex == 3  ) {
	FrecuencyHV->Text = 250 ; }
  else   {
	FrecuencyHV->Text = 1000;
  }
