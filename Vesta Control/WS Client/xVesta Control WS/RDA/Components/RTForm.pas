...
begin
  RT_Control := ShellClient.Control.InControl;
  RT_Enabled := ShellClient.Video.Video.RTEnabled;

  if RT_Enabled and ( not ShellClient.Acquiring )
    then RTCellCount.Text := IntToStr( (ShellClient.VestaRDA.TxsRxs as IVideo).RTCellCount );
...
begin
  ShellClient.ConnectDisconnectRT;
end;
...
begin
  try
    ShellClient.StartStopAcquire( StrToInt( RTCellCount.Text ) );
    UpdateView;
  except
  end;
end;
...
begin
  ...
  ShellClient.ViewCh1 := fData.Channel1;
  ShellClient.ViewCh2 := fData.Channel2;
  //Client
  Edit2.Text := IntToStr( fData.CellSize );
  Edit3.Text := IntToStr( fData.CellCount );
...
begin
  ...
  begin
    ...
    Channel1        := ShellClient.ViewCh1;
    Channel2        := ShellClient.ViewCh2;
    DB              := RTDisplay.DB_DBZ;
...
begin
  ShellClient.ViewCh1 := RTCh1.Checked;
end;

procedure TRealTimeForm.RTCh2Click(Sender: TObject);
begin
  ShellClient.ViewCh2 := RTCh2.Checked;
end;
