...
uses
  SysUtils, Forms, StdCtrls, ExtCtrls, Controls, Graphics, Classes, InvokeRegistry, ConfigurationWS, CommonObjs;
...
type
  ...
  public
    procedure Init( Value : IConfigurationWS; AuthHeader : TAuthHeader);
      ...
      if Assigned(Configuration)
        then
          begin
            ...
            LDate.Caption := 'Posicion del Sol [' + DateTimeToStr( SystemTime )  + ']';
...
begin
  ...
  try
    try
      RadarLatitude := Configuration.Get_RadarLatitude(Configuration.Radar_ID);
      RadarLongitude := -1 * Configuration.Get_RadarLongitude(Configuration.Radar_ID);
      RadarAltitud := Configuration.Get_RadarAltitude(Configuration.Radar_ID);
      SystemTime := ShellClient.VestaRDA.SystemDateTime;
      TimeZone := ShellClient.VestaRDA.SystemTimeZone;
      SunRise( SystemTime );
      SunSet( SystemTime );
...
begin
 try
    ...
    if (spa_calculate( YearOf( aDateTime ), MonthOf( aDateTime ), DayOf( aDateTime ),
                       ...
                       Configuration.RadarPressure,  // Presión media anual de la estación (hPa, me la dió Ihomaris)
                       Configuration.RadarTemperature,  // Temperatura media anual de la estación (grados celcius, me la dió Ihomaris)
...
function TFrame_SunPosition.GetConfiguration: IConfigurationWS;
var
...
procedure TFrame_SunPosition.Init(Value: IConfigurationWS; AuthHeader: TAuthHeader);
begin
  fUpdating := false;
