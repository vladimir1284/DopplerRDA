...
type
  ...
  private
    ...
    function OpenDPlayWithoutLobbyServer: Boolean;
    function OpenDPlayWithoutLobbyClient(out Name: string): Boolean;
  ...
  public
    ...
    procedure Close;
    procedure Open;
    procedure OpenServer( const ServerName : string );
    ...
    property SecName : string read fSecName write fSecName;
  end;
...
procedure TCustomDXPlay.OpenServer( const ServerName : string );
var
  PlayerName: string;
begin
  Close;
  try
    if not OpenDPlayWithLobby(PlayerName) then
    begin
      if not OpenDPlayWithoutLobbyServer then
        Abort;
    end;
    if ServerName <> ''
      then PlayerName := ServerName
...
function TCustomDXPlay.OpenDPlayWithoutLobbyServer: Boolean;
begin
  with TDelphiXDXPlayForm.Create(Application) do
  try
    DXPlay := Self;
    FormShow( nil );
    ProviderList.Selected[ ProviderList.Items.IndexOf( 'Conexi TCP/IP de Internet para DirectPlay' ) ] := true;
    NotebookPageChanged( nil );
    NextButtonClick( nil );
    NewGame.Checked := true;
    NextButtonClick( nil );
    if DXPlay.SecName <> ''
     then NewGameSessionName.Text := DXPlay.SecName
     else exit;
    NextButtonClick( nil );
...
procedure TCustomDXPlay.OpenClient;
var
  PlayerName: string;
begin
  Close;
  try
    if not OpenDPlayWithLobby(PlayerName) then
    begin
      if not OpenDPlayWithoutLobbyClient(PlayerName) then
        Abort;
    end;
    PlayerName := 'RTClient';
    Open_(PlayerName);
...
function TCustomDXPlay.OpenDPlayWithoutLobbyClient( out Name: string): Boolean;
begin
  with TDelphiXDXPlayForm.Create(Application) do
  try
    DXPlay := Self;
    ...
    JoinGameSessionList.Selected[ JoinGameSessionList.Items.IndexOf( 'RealTime' ) ] := true;
    JoinGameSessionListClick( nil );
    JoinGamePlayerName.Text := 'RDAClient';
    NextButtonClick( nil );
