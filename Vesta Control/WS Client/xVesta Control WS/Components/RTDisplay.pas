...
uses
  Windows, SysUtils, Classes, Controls, StdCtrls, Types, Graphics, Dialogs, Messages, Themes, Scale;
...
type
  ...
  private
    ...
    procedure SetDB_DBZ(const Value: boolean);
    ...
    function  Small2Big( Src : PBytes; SSize, DSize, SCount, DCount : cardinal ): PBytes;
    function  Big2Small( Src : PBytes; SSize, DSize, SCount, DCount : cardinal ): PBytes;
    function  StretchCells( Src : PBytes; SrcSize, DstSize, SrcCount, DstCount : cardinal ) : PBytes;
  ...
  public
    ...
    procedure AddBeam( Az, El : double; RawCellSize, RawCellCount : integer; Data : PBytes );
    procedure AddBeamWithoutPaint( Az, El : double; RawCellSize, RawCellCount : integer; Data : PBytes );
  ...
  published
    ...
    property DB_DBZ : boolean read fDB_DBZ write SetDB_DBZ;
...
procedure TRTDisplay.AddBeam(Az, El: double; RawCellSize, RawCellCount: integer; Data: PBytes);
begin
...
function TRTDisplay.Big2Small(Src: PBytes; SSize, DSize, SCount, DCount: cardinal): PBytes;
var
  I, J : cardinal;
  S, D : cardinal;
begin
  GetMem( result, DCount );
  FillChar( result^, DCount, 0 );
  I := 0;
  J := 0;
  S := SSize;
  D := 0;
    ...
    begin
      ...
      if (D - S) >= DSize
        then result[J] := Src[I]
          ...
          if SCount - I > 0
            then result[J] := (Src[I] * (D - S) + Src[I + 1] * (DSize - (D - S))) div DSize
            else result[J] := (Src[I] * (D - S)) div DSize;
      inc(J);
      inc(D, DSize);
    end;
end;
...
begin
  if fDB_DBZ
    then result := DefaultDBScale
    else result := DefaultDBZScale;
end;
    ...
    then
      begin
        ...
        begin
          ...
          if fDB_DBZ
            then
              Canvas.TextOut( Zone.Left + (((Zone.Right - Zone.Left)-(Canvas.TextWidth( IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDB ))) ))) div 2),
                              Zone.Top + (((Zone.Bottom - Zone.Top)-(Canvas.TextHeight( IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDB ))) ))) div 2),
                              IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDB ))) )
            else
              Canvas.TextOut( Zone.Left + (((Zone.Right - Zone.Left)-(Canvas.TextWidth( IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDBZ ))) ))) div 2),
                              Zone.Top + (((Zone.Bottom - Zone.Top)-(Canvas.TextHeight( IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDBZ ))) ))) div 2),
                              IntToStr( Trunc(CodeMeasure( fColorScale.Value[ i ], unDBZ ))) )
        end;
        Canvas.Pen.Color := fBackgroundColor;
        Canvas.Brush.Color := fBackgroundColor;
...
function TRTDisplay.Small2Big(Src: PBytes; SSize, DSize, SCount, DCount: cardinal): PBytes;
var
  Area, SIndex, DIndex, S, D  : cardinal;
begin
  GetMem( result, DCount );
  FillChar( result^, DCount, 0 );
  S := 0;
  ...
  begin
    Area := 0;
    if SIndex < SCount
      then Inc(Area, Src[SIndex] * (SSize-(D-S)) )
      else exit;
    inc(SIndex);
    inc(S, SSize);
    inc(D, DSize);
    ...
    begin
      if SIndex < SCount
        then Inc(Area, Src[SIndex] * SSize )
        else exit;
      Inc(SIndex);
      Inc(S, SSize);
    end;
    if SIndex < SCount
      then inc(Area, Src[SIndex] * (D - S) )
      else exit;
    result[DIndex] := Area div DSize;
    Inc(DIndex);
  end;
end;

function TRTDisplay.StretchCells(Src: PBytes; SrcSize, DstSize, SrcCount, DstCount: cardinal): PBytes;
begin
  if SrcSize < DstSize
...
procedure TRTDisplay.AddBeamWithoutPaint(Az, El: double; RawCellSize, RawCellCount: integer; Data: PBytes);
var
  i : integer;
  Angle, DBZ : double;
...
begin
  ...
  Buffer := StretchCells( Data, ProjectedCellSize, fCellSize, RawCellCount, DisplayCellCount );
  ...
  if not fDB_DBZ
    then
      ...
      if Buffer[i] > 0
        then
          begin
            DBZ := Buffer[i] + (20 * Log10( (i+1)*fCellSize/1000 )) + fPotMet;
            Buffer[i] := MeasureCode( DBZ, unDBZ );
          end
...
procedure TRTDisplay.SetDB_DBZ(const Value: boolean);
begin
  fDB_DBZ := Value;
  ClearBeams;
  if csDesigning in ComponentState
